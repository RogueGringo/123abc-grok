...
def write_partner_science_pdf(
    ...
    stamp = annex.get("stamp") or root.name

    # Brand header bar
    pdf.set_fill_color(18, 32, 56)
    pdf.rect(0, 0, 210, 22, style="F")
    pdf.set_xy(16, 6)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 6, "DUAL-GATE  |  KNOWN-SOLUTIONS SCIENCE", ln=1)
    pdf.set_x(16)
    pdf.set_font("Helvetica", "", 9)
    pdf.cell(
        0,
        5,
        _safe(f"soft_T(n=12)={soft}  |  pin_ok={pin.get('ok')}  |  stamp={stamp}"),
        ln=1,
    )
    ...
    h("Partner science one-pager", 13)
    muted(
    ...
    pdf.rect(16, y0, 178, 28, style="F")
    pdf.set_xy(18, y0 + 2)
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 6, "Locked dual-gate pin", ln=1)
    pdf.set_x(18)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 5, _safe(f"pin ok: {pin.get('ok')}    soft_T(n=12): {soft} (expect 0.036)"), ln=1)
    pdf.set_x(18)
    pdf.cell(
        0,
        5,
        _safe(
            f"seq_mix: {pin.get('seq_mix', 0.0)}    face_weight: {pin.get('face_weight', 0.08)}"
        ),
        ln=1,
    )
    pdf.set_x(18)
    pdf.cell(0, 5, _safe(f"stamp: {stamp}"), ln=1)
    pdf.set_y(y0 + 30)
    pdf.set_x(16)
    ...
    pdf.cell(
        ...
        4,
        _safe(
            "Commercial success = openable PDBs + dual-gate pin  |  "
            "Science annex never gates SHIP  |  never lambda=gamma"
        ),
        ln=1,
    )
    pdf.set_x(16)
    pdf.cell(
        0,
        4,
        _safe(f"Verify proof zip: python handoff_ship.py --verify-bundle partner_receipts_*.zip"),
        ln=1,
    )
